...
public class assembler {
    ...
    public static void main(String[] Args) throws IOException {
        ...
        while ((s = b1.readLine()) != null) {
            ...
            if (word[0 + offset].compareToIgnoreCase("STOP") != 0) {
                ...
                if (checkerVal == 0) {
                    ...
                    if (word[0 + offset].compareToIgnoreCase("EQU") == 0
                            || word[0 + offset].compareToIgnoreCase("ORIGIN") == 0) {
                        String s1 = "";
                        int i = 0, val, op2 = 0;
                        char sign = ' ';
                        while (i < word[1 + offset].length() && (Character.isAlphabetic(word[1 + offset].charAt(i))
                                || Character.isDigit(word[1 + offset].charAt(i)))) {
                            s1 += word[1 + offset].charAt(i);
                            i++;
                        }
                        if (i != word[1 + offset].length()) {
                            op2 = Integer
                                    .parseInt(word[1 + offset].substring(s1.length() + 1, word[1 + offset].length()));
                    ...
                    if (word[0 + offset].compareToIgnoreCase("BC") == 0) {
                        ...
                        if (symNumber.containsKey(symbols[1])) {
                            f4.write("(" + ((int) (word[1 + offset].charAt(0)) - 64) + ")(S,"
                                    + symNumber.get(symbols[1]) + ")\n");
                        } else {
                    ...
                    if (word[0 + offset].compareToIgnoreCase("BC") == 0) {
                        ...
                        if (litNumber.containsKey(symbols[1])) {
                            f4.write("(" + ((int) (word[1 + offset].charAt(0)) - 64) + ")(L,"
                                    + litNumber.get(symbols[1]) + ")\n");
                        } else {
        ...
        for (int i = 1; i < symtabPointer; i++)
            f3.write(symSymbol.get(i) + "\t\t\t" + symAddress.get(symSymbol.get(i)) + "\t\t\t"
                    + symLength.get(symSymbol.get(i)) + "\n");
        if (litNumber.size() != 0) {
            for (int i = pooltabPointer, j = 0; j < litSymbol.size(); i++, j++) {
                f4.write("(DL,01)(C," + Integer.parseInt(litSymbol.get(i)) + ")\n");
                f5.write(litSymbol.get(i) + "\t\t" + addr++ + "\n");
